...
#Upto what sample numbers is it considered a significant voice or silence?
#Change this value to change the threshold.
def isNotSignificant(sample1, sample2):
    return (abs(sample2-sample1) < 44100*(2/5))
...
# isNotSignificant() function
def getRidOfSmallDisruptions(List):
    FirstList = []
    i = 0
    while (i< len(List)-1):
        if (isNotSignificant(List[i+1][0],List[i][1])):
            #merge the two
            FirstList.append((List[i][0], List[i+1][1]))
            i = i+1
        else:
            FirstList.append((List[i][0], List[i][1]))
        i = i + 1
    if ((FirstList[-1][1] != List[-1][1])):
        FirstList.append(List[-1])

    ReturnedList = []
    i = 0
    while (i< len(FirstList)-1):
        if ((FirstList[i+1][0] -FirstList[i][1]) < 11025):
            #merge the two
            ReturnedList.append((FirstList[i][0], FirstList[i+1][1]))
            i = i+1
        else:
            ReturnedList.append((FirstList[i][0], FirstList[i][1]))
        i = i + 1
    if ((ReturnedList[-1][1] != FirstList[-1][1])):
        ReturnedList.append(FirstList[-1])
    return ReturnedList
...
# calculates, returns and prints(optional) the Pause Interval times
# for both Persons.
...
#Person* = (List of Silence Intervals,
           ...
           #List of pause Intervals) <== For now
